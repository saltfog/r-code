...
library(readr)
bothtags <- read_csv("~/r-code/ fishthinkers /Bothtags.csv")
View(bothtags)
...
p3 <- bind_rows(bothtags$`Distance(m)`,bothtags$`Tag 1`,bothtags$`Tag 2`)

p3 <- ggplot(bothtags, aes(x=bothtags$`Distance(m)`, y=bothtags$`Tag 2`)) +
  geom_smooth(method = 'loess') +
  geom_point() +
  labs(x="Distanct", y="Tag", title="Range Tag 1 and 2") + 
