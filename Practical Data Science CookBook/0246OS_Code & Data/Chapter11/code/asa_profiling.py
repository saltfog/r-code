...
def calculate_asa(atoms, probe, n_sphere_point=960):
    ...
    #r = np.max(radii)
    #test_r_sq = (r + r + probe + probe ) ** 2
    #neighbor_dist_sq = cdist(points, points, 'sqeuclidean')
    #neighbor_boolean = (neighbor_dist_sq < test_r_sq)
    ...
    for i in xrange(0, num_atoms):
        ...
        neighbor_indices = find_neighbor_indices(points, radii, probe, i)
        #neighbor_indices = neighbor_boolean[:,i]
        #neighbor_indices[i] = False

        #radius = np.array(probe) + radii[i]
        ...
        #diff_sq = np.amin(diff_sq,1)
        #print(diff_sq.shape)

        #areas[i] = np.sum(diff_sq<neighbor_radii_sq)

        dist_test = (diff_sq < neighbor_radii_sq)
        #print(dist_test.shape)
        
        #inaccessible_points = np.sum(dist_test,1)
        inaccessible_points = np.any(dist_test,1)
        #print(inaccessible_points.shape)

        #area = sum(inaccessible_points==0) * const * radius * radius
        #areas[i] = (n_sphere_point - np.count_nonzero(inaccessible_points)) * const * radius * radius
        #areas[i] = np.count_nonzero(inaccessible_points)
